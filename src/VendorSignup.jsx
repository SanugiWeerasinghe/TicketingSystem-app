...
const VendorSignup = () => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:8080/api/vendor/signup', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '400px', margin: '0 auto', backgroundColor: '#f4f4f4', borderRadius: '8px' }}>
      ...
      <p style={{ textAlign: 'center', marginTop: '10px' }}>
        Already have an account? <a href="/vendor/login" style={{ textDecoration: 'none', color: '#0044cc' }}>Login</a>
      </p>
    </div>
  );
};
